...
bool Player::place(Board& playedBoard, Dictionary myDict, Bag& bag) {
	string dir;
	string letters;
	string buffer = "";
	// char letterWanted;
	...
	cin >> row;
	cin >> col;
	cin >> letters; // string of letters to put
...
int Player::move(string currentMove, Bag& bag, Board& playedBoard, Dictionary myDict) {

	if(currentMove == "PLACE") {
		if(place(playedBoard, myDict, bag)) return 1;
			else {
				cout << "Invalid Place. Please try again." << endl;
				return 0;
			}
	} else {
			if(exchange(bag)) return 2;
			else {
